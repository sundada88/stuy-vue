...
export default {
  ...
  computed: {
    visibleData () {
      ...
      const renderStart = this.renderStart = this.start - prevCount // 真正的渲染开始项
  ...
  mounted () {
    ...
    // 计算可视区域的高度
    this.$refs.wrap.style.height = this.arrayData * this.size + 'px'
    // 缓存每一项的高度
  ...
  updated () {
    this.$nextTick(() => {
      ...
      console.log('this.$refs.scrollHeight.style.height ===> ', this.$refs.scrollHeight.style.height)
    })
  },
  methods: {
    handleScroll () {
      ...
      if (this.variable) { // 每一项高度不固定
        ...
        console.log('this.start', this.start - this.prevCount)
        ...
        this.offset = this.positionListArr[this.start - this.prevCount] ? this.positionListArr[this.start - this.prevCount].bottom : 0
        // this.offset = this.positionListArr[this.start - this.prevCount] ? this.start - this.prevCount === 0 ? 0 : this.positionListArr[this.start - this.prevCount].bottom : 0
        // this.offset = this.scrollTop
        // console.log("this.start ===> ", this.start)
        // if (this.positionListArr[this.renderStart]) {
        //   let count = this.positionListArr[this.renderStart].top
        //   let i = 0
        //   const setOffset = (count) => {
        ...
        //   setOffset(count)
        // }
        this.offset = this.positionListArr[this.renderStart] ? this.positionListArr[this.renderStart].top : 0
      } else { // 每一项高度固定
    ...
    getStartIndex (scrollTop) {
      ...
      let end = this.positionListArr.length - 1 // positionListArr 的最后一项的下标
      // 用 temp 存储得不到精确值时最终返回的值，因为可能滚动的距离的值不会等于 positionListArr 数组里
      // 任何一项的 bottom 的值，所以只好存储一个最接近的值
      let temp = null
      while (start <= end) {
        const midIndex = parseInt((end + start) / 2) // 中间那一项的 index，也可以用 Math.floor 代替 parseInt
        const midVal = this.positionListArr[midIndex].bottom // 中间那一项的 bottom 值
        if (scrollTop === midVal) {
