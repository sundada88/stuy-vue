<template lang="html">
  <div class="vue-list" @scroll="handleScroll">
    ...
    >
      ...
      <li v-for="item in previewList" :key="item.id">{{ item.title }}</li>
...
export default {
  ...
  watch: {
    previewList (newVal) {
      console.log('previewList ----> ', newVal)
    }
  },
  data () {
    return {
  ...
  methods: {
    ...
    handleScroll () {
      ...
      const _height = this.$el.querySelectorAll('ul')[0].offsetHeight
      const _contentHeight = this.$el.offsetHeight
    ...
    loadmore (from, to) {
      ...
      setTimeout(() => {
        for (var i = 0; i < 200; i++) {
          this.list.push({
            title: 'item ' + this.list.length++
          })
        }
        const _from = from; const _to = to + this._below
        this.resetPreviewList(_from, _to)
        this.lineBottomHeight = (this.list.length - _to) * this.height
        this.handleScroll()
